...
class Sensors():
    def __init__(self, is_simulator=False, use_back_sensors=False):
        ...
        self.crash_subscriber = rospy.Subscriber("/crash", Bool, self.crash_callback)
    ...
    def set_no_crash(self):
        self.is_crash = False

    def get_is_crash(self):
        return self.is_crash

    def get_lidar_ranges(self):
        if not self.is_simulator:
            return self.lidar_data.ranges[:-1]  # Real car has 1081 rays
        else:
    ...
    def get_car_position(self):
        return (self.odometry.pose.pose.position.x, self.odometry.pose.pose.position.y)
