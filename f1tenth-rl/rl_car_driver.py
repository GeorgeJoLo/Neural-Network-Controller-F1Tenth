...
parser.add_argument("--epsilon-decay", type=float, default=0.9997, help="]0, 1] every step epsilon = epsilon * decay, in order to decrease constantly")
parser.add_argument("--epsilon-min", type=float, default=0.05, help="epsilon with decay doesn't fall below epsilon min")
...
parser.add_argument("--reduce-lidar-data", type=int, default=1, help="lidar data are grouped by taking the min of [reduce-lidar-data] elements")
...
parser.add_argument("--lidar-reduction-method", choices=['avg', 'max', 'min', 'sampling', 'moving_avg'], default='moving_avg', type=str.lower, help="method used to aggregate lidar data")
parser.add_argument("--moving-avg-window", type=int, default=9, help="smd")
...
while environment.get_game_number() < 7000:
