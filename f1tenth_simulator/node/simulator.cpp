...
public:
    ...
    RacecarSimulator(): im_server("racecar_sim") {
        ...
        // Make a publisher if the car crashes
        crash_pub = n.advertise<std_msgs::Bool>("/crash", 1);
        ...
        ROS_INFO("Simulator constructed. AAA");
    }
            ...
            if (state.velocity != 0) {
                for (size_t i = 0; i < scan_.size(); i++) {
                    ...
                    if ((ttc < ttc_threshold) && (ttc >= 0.0)) {
                        ...
                        std_msgs::Bool is_crash;
                        is_crash.data = true;
                        crash_pub.publish(is_crash);
